...
* **Pseudo Analog Output (PA9):** PWM-based average voltage output (0–3.3V), usable as a DAC substitute with an external RC low-pass filter (R = 4k7–10k, C = 1–10µF).
...
* **Emergency Stop System (PC14):** Hardware-driven emergency stop with immediate PWM flush (both outputs forced to 0V simultaneously via EGR register), timer pause, onboard LED (PC13) blink alert at 500ms, and <ACK> confirmation requirement before the system can resume.
...
Button Mapping
DefinePinFunctionSEL_ESCPA0Confirm, enter sub-menu, save settings, ACK, escapeNAV_ADJPA2Scroll menu, adjust value, toggle Run/Stop
...
/Screenshots      — LCD menu screenshots and hardware photos
```
...
* **Home Screen:** Displays system run state and a live incrementing counter (updated every 250ms by TIM2 ISR).
...
* **Motor Speed (Menu 3):* Short press NAV_ADJ steps the parameter by 3 units (≈1.2%). Long press (2s) flips direction (+ / −) and enables auto-scroll at 100ms intervals. Parameter is safely clamped to [0, 255] — no wrap-around, preventing dangerous power jumps on real loads.
* **Save Settings (Menu 3 → SEL_ESC):** Writes current parameters to Flash (pseudo EEPROM) with interrupt suspension for write safety.
* **Voltage Monitor (Menu 4):** Reads PA1 ADC in real time and displays the mapped voltage level (0.00–3.26V).
* **Emergency Stop:** Triggered by PC14 going LOW. Both PWM outputs are flushed to 0V simultaneously, the timer is paused, and the LCD locks on EMERGENCY STOP / <ACK>. The onboard LED blinks at 500ms. System resumes only after the physical EMS is released AND the operator confirms with SEL_ESC.
